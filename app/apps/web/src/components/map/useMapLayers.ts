 ...
 *
 * Owns flood zones and the congestion choropleth (flood under congestion).
 * The `buildings` and `agents` toggles are owned by the pages (PolygonLayer /
 * TripsLayer) and are ignored here, as is any unknown toggle id — the
 * LayerLegend can grow additively without touching this hook.
 *
 ...
 *
 * Per-run caching note: callers hold the data (edge counts, GeoJSON) in
...
export function useMapLayers(
  ...
  const { congestion, flood } = toggles ?? {};
  const { edgesGeoJSON, edgeCounts, baselineCounts, floodGeoJSON } = data ?? {};
