...
# context); best-available *open* estimate, NOT a live survey -> Behavioral *behavior*
# confidence = M (READINESS soft spot; methods §3.1, §4; CR-016 open-data-only).
...
#
# OPEN-DATA POLICY (CR-016): Government FOI / household surveys are not feasible.
# Do **not** invent newer shares. VAL-01 corridor volumes stay directional (FAIL
# expected under Tier-B WorldPop demand — never fit to Calderon passenger_flow_max).
#
# OPTIONAL OPEN UPDATE: if a newer *open-licensed* published mode-share table for
# Iloilo / Region VI appears, document vintage in data/INVENTORY.md then inject:
#   MATRIX_MODE_SHARE='{"jeepney":0.XX,...}' (must sum to ~1.0) before API start.
