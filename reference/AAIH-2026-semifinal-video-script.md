...
> "I'm a city planner. No GIS training, no modeling background. I just describe the project —" *(type)* "— *'an eight-storey mixed-use building here, beside the Esplanade.'* Gemini 3.1 parses that into a simulation plan. Watch."
...
> "This isn't a video — it's a live SUMO simulation. Hundreds of commuter agents, each a persona generated by Gemini Flash-Lite, routing through the real road network. The animation starts *while the impact modules are still computing* — that's the streaming architecture. The motion you're seeing is the actual simulated reality all five dimensions are about to score."
...
3. Confirm `GEMINI_API_KEY` is set and a quota check passes (don't get rate-limited on camera).
