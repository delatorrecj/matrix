...
| 0.3 | Local-first datastores: ChromaDB (local), Postgres+PostGIS (local; in-memory fallback in prod), Redis (local). Cloud provisioning deferred to Phase 4 unless trivial. | Jerico | `.env` template; never commit secrets ([clr-matrix.md](clr-matrix.md)). |
...
| 4.5 | Cloud deploy: Hugging Face Spaces (FastAPI + SUMO + Redis in one self-contained Docker container; in-memory persistence) + Vercel frontend; SUMO baseline seeded on boot (CR-011) | Jerico | First time the cloud path is exercised end-to-end. |
