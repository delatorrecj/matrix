...
**Status:** Phase 1 implemented (branch `feat/cr010-ui-humanization`); Phase 2 implemented (branch `feat/cr010-phase2`, PR pending merge)
**Date opened:** 2026-06-24
**Owner:** Carlos (delatorrecj)

> **Implementation status (2026-06-24).** Phase 1 (frontend-only) is built on branch `feat/cr010-ui-humanization`: number formatter + metric registry, plain-language Summary dock, dedicated interpreted Analytics view, nav-rail cleanup (logo→home, disabled-with-reason items, AU avatar removed, dead Help removed), and a minimal real Settings (theme + language). Verified by `next lint` (clean) and `tsc --noEmit` (clean for all changed files; the only `tsc` errors are pre-existing deck.gl accessor-typing ones in `tests/unit/mapLayers.test.ts`, excluded from `next build`). `next build`/`next dev` OOM **locally** (physical-RAM exhaustion bundling deck.gl/maplibre — a known local limit; CI is the build gate).
>
> **Phase 2 (2026-06-24, branch `feat/cr010-phase2`):** WS-5 implemented — (T5.1) the kernel synthesis prompt is rewritten to the plain-language BLUF structure (`synthesis.py`), keeping the inline `[EQN-ID]` citation contract; (T5.2) the brief is **bilingual by `=== HILIGAYNON ===` delimiter** (kernel constant `HILIGAYNON_MARKER`), consumed by the web `lib/bilingual.ts` + `SynthesisNarrative`/`SummaryView` via the Phase-1 `useLanguage()`; (T5.4) a dedicated print-scoped one-page brief (`ScenarioBrief.tsx`) replaces the panel-wide `window.print()`, with a HEADLINE→…→KEY RISK body + an EVIDENCE appendix (equation ids + datasets); (T5.3) the **Locked** `methods-matrix.md` §4/§4.3 and `prd-matrix.md` PRD-F7 are amended and **flagged PENDING RE-LOCK under CR-010** (not auto-approved). The citation guard was tightened to split on newlines (BLUF briefs are newline-structured) while preserving layout. Verified locally: kernel `python -m pytest -q` → 182 passed / 11 skipped; API `python -m pytest -q` → 64 passed / 4 skipped; web `next lint` clean + `tsc --noEmit` clean for all changed files. vitest (new `bilingual.test.ts`) + Playwright e2e + `next build` run in **CI** (flaky/OOM locally per the dev-env notes).
