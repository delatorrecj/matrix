...
> **Edge-resolution fallback (2026-06-25, PR #37, merged).** Fixed the "identical -14 trips" defect in production QA: `_resolve_edges` in `runner.py` now consults the gazetteer before keyword-match and, when nothing resolves, picks among the top-50 busiest baseline edges via a deterministic MD5 hash of the location string (instead of always the single busiest edge). Provenance preserved (`gazetteer-match`, `gazetteer-match (PROVISIONAL-id)`, `busiest-baseline-fallback (deterministic-hash; …)`). methods §4.2 updated + re-locked; landing page unaffected; no validation-gate change. CI green (kernel/api/web/e2e).
