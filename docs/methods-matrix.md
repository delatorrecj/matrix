...
**Kernel edge resolution (`_resolve_edges`).** The simulation kernel resolves *where* a scenario applies independently of the LLM orchestrator path above. Order in [`runner.py`](../app/packages/kernel/matrix_kernel/runner.py): (1) map-drop geometry → network edges; (2) **gazetteer-match** on `scenario.effective_location` via `resolve_colloquial_term` (method string includes `(PROVISIONAL-id)` when the gazetteer entry is provisional); (3) **keyword-match** on street names in the cached SUMO net; (4) **busiest-baseline-fallback** — if a location string is present but unmatched, a deterministic MD5 hash of the location selects among the top-50 busiest baseline edges (`busiest-baseline-fallback (deterministic-hash; …)`), so distinct unknown locations produce distinct module outputs instead of collapsing to the single busiest edge (the defect fixed in PR #37). When no location is given, the honest last resort remains the single busiest edge (`busiest-baseline-fallback (no location given)`). The method string is recorded verbatim in `Trajectory.meta` (PRD-F14).
