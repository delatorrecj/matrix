...
> **Gazetteer live-net verification (CR-018).** `gazetteer_iloilo.json` aliases are verified against the deployed SUMO net: live `sumo_edge`/`sumo_edges` membership, OSM lane `origId`, curated `street_name` alias, then capped coordinate snap (`gazetteer-snap`). A hit is annotated `PROVISIONAL-id` only while `"provisional": true`. Districts resolve to a plaza-street proxy, not a barangay polygon. This does **not** change confidence letters (VAL-01 / dataset tiers still govern those).

**Example: Hiligaynon Query**
- **Query:** *"Ano matabo kung siraduhon ang merkado kag ang tulay sa forbes?"*
- **Gazetteer Pre-processing:** The term "merkado" triggers a hit for `Iloilo Central Market` (street alias Valeria), and "tulay sa forbes" hits `Forbes Bridge` (coordinate snap on the live net).
- **Query Annotation:** The orchestrator annotates the query before sending it to the LLM with the curated canonical name + OSM/SUMO ids from the gazetteer (never invented by the model).
...
**Kernel edge resolution (`_resolve_edges`).** The simulation kernel resolves *where* a scenario applies independently of the LLM orchestrator path above. Order in [`runner.py`](../app/packages/kernel/matrix_kernel/runner.py) + [`span.py`](../app/packages/kernel/matrix_kernel/span.py): (1) map-drop geometry → network edges; (2) gazetteer on `scenario.effective_location` via `resolve_colloquial_term`: **gazetteer-match** (ids that exist in the live net; `(PROVISIONAL-id)` only if still unverified), **gazetteer-osmid** (OSM way `origId`), **gazetteer-alias** (`street_name` keyword), **gazetteer-snap** (nearest live edges within a feature-type radius, capped); (3) **keyword-match** on live `edge.getName()` after name normalization (`St.`/`EL98`, token match — a stuffed span sentence is not a match); (4) if `from_cross` and/or `to_cross` (in `Scenario.parameters`) resolve to junctions on that named corridor, **keyword-span** (both crosses: shortest-path edges, both directions) or **keyword-span-open** (one cross → far end of the named street). Crosses that miss the corridor keep the whole-street set, method stays **keyword-match**, assumption `span_crosses_off_corridor`. An empty walk does not fall through to the hash while a real corridor name is in hand; (5) if step 3 misses, the longest live street name that is a substring of `location` or `description`; (6) **busiest-baseline-fallback** — if a location string is present but unmatched, a deterministic MD5 hash of the location selects among the top-50 busiest baseline edges (`busiest-baseline-fallback (deterministic-hash; …)`), so distinct unknown locations produce distinct module outputs instead of collapsing to the single busiest edge (the defect fixed in PR #37). When no location is given, the honest last resort remains the single busiest edge (`busiest-baseline-fallback (no location given)`). The method string is recorded verbatim in `Trajectory.meta` (PRD-F14) with `from_cross` / `to_cross` / `span_nodes`. Closed-edge LineString shapes (`affected_edge_geoms`) ride the playback payload so the magenta overlay follows closed ids, not only baseline-trafficked features in `public/layers/edges.geojson`. `location` is the corridor street name only — the LLM never emits OSM/SUMO ids or coordinates.
