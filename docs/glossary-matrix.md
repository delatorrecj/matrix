...
| **Gazetteer** | A curated lookup mapping local/Hiligaynon place names (e.g. "tulay sa forbes") and Iloilo district names (e.g. "Molo") to OSM/SUMO ids *before* the LLM sees the query — the LLM never originates a GIS id. **Most entries are PROVISIONAL** (`gazetteer_iloilo.json`). When `sumo_edge` is missing from the live net, an optional `street_name` alias is keyword-matched instead (Molo → Avanceña Street; Diversion Road → Aquino Jr). `GET /scenario` may still return `coordinates`; the results-map camera does not use them. |
