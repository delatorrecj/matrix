...
| **Gazetteer** | A curated lookup mapping local/Hiligaynon place names (e.g. "tulay sa forbes") and Iloilo district names (e.g. "Molo") to OSM/SUMO ids *before* the LLM sees the query — the LLM never originates a GIS id. Canonical OSM street names are **not** copied into the gazetteer; they resolve from the live net’s `edge.getName()` (**keyword-match**). A “from X up to Y” query is a **span** (`from_cross` / `to_cross` → **keyword-span**), not a longer `location` string. Kernel resolution (CR-018 / CR-020): live `sumo_edge`/`sumo_edges` membership, then OSM `origId`, then `street_name` alias, then capped coordinate snap, then live-net street index, then optional span clip. Unmatched names still hash to a busiest baseline edge, labeled `busiest-baseline-fallback`. Districts are a plaza-street proxy, not a polygon. `GET /scenario` may still return `coordinates`; the results-map camera does not use them. |
| **Location of interest** | Kernel `[lon, lat]` (first honest affected-edge midpoint) on `/simulate` `EDGE_COUNTS`; `null` on fallback. **Not** `Scenario.geometry`, **not** a TraCI edge id, and **not** a map glyph. The results-map camera ignores this point and `GET /scenario` coordinates; it flies only to the **corridor box** (padded bbox of honest affected edges) or stays on the city default. |
