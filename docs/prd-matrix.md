...
| PRD-F2 | **NL + map-drop scenario input** | Azure OpenAI GPT-5.4 orchestrator parses a natural-language query or a map-placed project into a simulation plan. | Must-Have |
...
| PRD-F7 | **Synthesis + recommendation report** | Azure OpenAI GPT-5.4 generates a plain-language **BLUF brief** (HEADLINE → WHAT WE SIMULATED → KEY FINDINGS → RECOMMENDATION → KEY RISK), **bilingual by delimiter** (English / Hiligaynon, toggle-selected — not inline-interleaved), with glossary tooltips, trend indicators, and a dedicated one-page exportable PDF brief carrying an evidence appendix (equation ids + datasets). Every number stays inline-cited (citation guard; glass box, PRD-F14). *(Amended & re-locked 2026-06-24 by **CR-010**, owner-approved.)* | Should-Have |
...
| Edge cases | data-sparse barangay (low confidence); SUMO/network gap; Azure OpenAI GPT-5.4 unavailable (see §7 fallback); multi-user load (queue) |
...
**Rollback plan:**
- *Trigger criteria:* demo-blocking failure (kernel/LLM outage, latency blowout) during a live session.
