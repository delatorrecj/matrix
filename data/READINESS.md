...
|---|---|---|---|
| **Engine / Base** | OSM network 12,579 ways ✅ · Overture segments 18,844 + connectors 18,893 + **148,630 buildings** ✅ · Overture/OSM land-use ✅ | **H** | DEM for gradients/flood (GLO-30 ⏳, PhilLiDAR ✋ — 10 m open) · **Phase 1.1 pipeline:** `app/packages/data/build_network.py` converts OSM-ILO + Overture → `iloilo.net.xml` + `iloilo.taz.xml` + `iloilo.rou.xml` in `app/packages/kernel/data/` (Stage 1 done; Stage 2 netconvert done; Stage 3 TAZ done. Network stats: 36,367 edges, 14,465 nodes) |
| **Behavioral** *(trip gen, mode shift, ped flow)* | road + transit network ✅ · OSM routes 54 + **LPTRP route URL index** (24 titles + source URLs — `raw/transport/routes.json`; no geometries) · **49 OSM PT relations** in `raw/osm/iloilo_osm.json` are the usable route geometry source ✅ · trip generators: Overture POIs 11,189 + OSM amenities + CCHAIN `osm_poi_*` ✅ · Calderon 2014 BRT calibration ✅ | **H** network / **M** behavior | **Phase 2.1 pipeline:** baseline vehicle demand generated via `randomTrips` and cached to `baseline:iloilo:latest` in Redis. **mode-share ground truth** is the soft spot — derived from literature, not a live travel survey → LTFRB FOI (`ltfrb-vi-foi.md` ✋) |
