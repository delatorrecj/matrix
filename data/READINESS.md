...
|---|---|---|---|
| **Engine / Base** | OSM network 12,579 ways ✅ · Overture segments 18,844 + connectors 18,893 + **148,630 buildings** ✅ · Overture/OSM land-use ✅ | **H** | DEM for gradients/flood (GLO-30 ⏳, PhilLiDAR ✋ — 10 m open) · **Phase 1.1 pipeline:** `app/packages/data/build_network.py` converts OSM-ILO + Overture → `iloilo.net.xml` + `iloilo.taz.xml` + `iloilo.rou.xml` in `app/packages/kernel/data/` (Stage 1 done; Stage 2 netconvert done; SUMO outputs regenerable, gitignored) |
